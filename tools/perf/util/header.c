			   ...
			   FILE *fp)
{
	...
	if (ph->env.perf_clock_ref) {
		if (localtime_r(&ph->env.tod_tv_ref.tv_sec, &ltime) == NULL)
			...
			snprintf(tstr, sizeof(tstr), "%s.%06ld",
				 date, ph->env.tod_tv_ref.tv_usec);
		}

		fprintf(fp, "# reference time: %s = %ld.%06ld = %" PRIu64 "\n",
			tstr, ph->env.tod_tv_ref.tv_sec, ph->env.tod_tv_ref.tv_usec,
