...
static int parse_nsec_time(const char *str, u64 *nsec)
{
	...
	*nsec = time_sec * NSEC_PER_SEC + time_nsec;
...
void perf_time__set_reftime(struct timeval *tv, u64 tref)
{
	...
	pr_debug("Updated ref time: %" PRIu64 " = %ld.%06ld\n",
		 tref, tv_ref.tv_sec, tv_ref.tv_usec);
}
